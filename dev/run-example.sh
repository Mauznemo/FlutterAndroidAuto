...
#
#   dev/run-example.sh              debug run, logs to /tmp/aa-example.log
#   dev/run-example.sh --release
#   dev/run-example.sh --bg         run detached, logging to that file
#   dev/run-example.sh --bundle     run the built binary directly, detached
#
...
if [ "$BUNDLE" = 1 ]; then
  ...
  echo "wait for 'Flutter run key commands' in the log before expecting a window"
else
