...
// under M4.
//
// So this exists to keep the phone happy, not to do its job:
//
//   sensors       answer the start request, then report "parked" and "day"     (M8)
//
// Input, the three audio sinks and the microphone used to be on that list. M5, M6 and M7
// gave them real implementations, so they moved out to session/input_channel.cc,
// session/audio_channels.cc and session/microphone_channel.cc, which is the shape this
// one is headed for too.
//
...
//
// M8 makes it a real service. Nothing here is meant to survive that.
...
// The relay below exists for the reason spelled out on ControlEventRelay: a channel
...
// the USB interface. It holds a weak reference.
...
class SupportChannels : public std::enable_shared_from_this<SupportChannels> {
 public:
  ...
  // Called by the relay, never by aasdk directly.
  void OnSensorOpen();
  void OnSensorStartRequest(
 ...
 private:
  // The live sensor channel, or nullptr if there is none or the session has stopped.
  //
  ...
  void ListenSensor();
  // Builds the channel under the lock and arms the receive outside it, which is the only
  // ordering that neither races Stop() nor takes the lock twice.
  void AddSensor();
  ...
  // Guards sensor_ and messenger_. Held for the length of a pointer copy, never across
  // a send.
